...
resource "aws_s3_bucket" "static_web" {
  bucket        = "vennila-staticwebsite-files" #Name of your bucket
  force_destroy = true
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  enabled             = true
  comment             = "vennila's static website using Cloudfront" #Description
  default_root_object = "index.html"

  aliases = ["ven.sctp-sandbox.com"] #Cloudfront alternate domain name (Same as Route53 record)
...
resource "aws_cloudfront_origin_access_control" "oac" {
  name                              = "ven-cloudfront-oac" #e.g. <yourname>-cloudfront-oac
...
resource "aws_route53_record" "www" {
  ...
  name    = "ven"  #Your domain prefix. <this-value>.sctp-sandbox.com
  type    = "A"
